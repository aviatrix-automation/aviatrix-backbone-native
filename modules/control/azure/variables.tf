...
variable "transits" {
  ...
  type = map(object({
    ...
    bootstrap_type         = optional(string, "file_share")
    panorama_dgname        = optional(string)
    panorama_tplname       = optional(string)
    panorama_cgname        = optional(string)
...
variable "spokes" {
  ...
  type = map(object({
    ...
    enable_max_performance           = optional(bool, true)
    disable_route_propagation        = optional(bool, false)
    vwan_connections = optional(list(object({
      vwan_name                     = string
      vwan_hub_name                 = string
      enable_learned_cidrs_approval = optional(bool, false)
...
variable "vwan_hubs" {
  ...
  type = map(object({
    ...
    propagate_default_route                = optional(bool, true)
  }))
  default = {}
}
...
variable "panorama_config" {
  description = "Panorama configuration for dynamic bootstrap."
  type = object({
    ...
    dgname              = string
    vm_auth_key         = string
    auth_key_ttl        = optional(string, "8760")
    cgname              = optional(string)
    plugin_op_commands  = optional(string)
    enable_dpdk         = optional(bool, true)
    mgmt_interface_swap = optional(bool, true)
    csp_pinid           = optional(string)
    csp_pinvalue        = optional(string)
  })
  default = null
